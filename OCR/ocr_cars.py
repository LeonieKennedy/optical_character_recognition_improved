...
class ExtractLicencePlatesModel:
    def __init__(self):
        ...
        # Load model
        self.model = cv2.dnn.readNetFromONNX('models/best.onnx')
    ...
    def detect_licence_plates(self, input_image):
        ...
        blob = cv2.dnn.blobFromImage(input_image,1/255,(self.INPUT_WIDTH, self.INPUT_HEIGHT), swapRB=True, crop=False) # 1/255 = scale factor
    ...
    def annotate_image(self, image,licence_coords,confidences_np,index):
        ...
        if licence_text == "number plate not detected" or "":
            print("no coords")
